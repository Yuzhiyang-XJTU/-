...
for epoch in range(1, num_epochs + 1):
    ...
    for batch in range(1, n_batch + 1):
        ...
        # print('*'*5)
        # optimizer.zero_grad()  # 梯度清零
        # transfer_loss.backward(create_graph=True)
        # tf_grad = torch.abs(net.conv1.weight.grad).sum()
        # print(tf_grad)
        # optimizer.zero_grad()  # 梯度清零
        # source_clf_loss.backward()
        # source_grad = torch.abs(net.conv1.weight.grad).sum()
        # print(source_grad)
        # print('比值：', tf_grad.item() / source_grad.item())
        # print('*' * 5)
    ...
    if acc >= Max_acc:
        # 保存网络参数
        PATH = 'Max_多层MMD_params.pth'
        torch.save(net.state_dict(), PATH)
        Max_acc = acc

    PATH = 'Last_多层MMD_params.pth'
    torch.save(net.state_dict(), PATH)
    ...
    np.save(r'tf_C1.npy', np.array(tf_C1))
    np.save(r'tf_C2.npy', np.array(tf_C2))
    np.save(r'tf_F1.npy', np.array(tf_F1))
    np.save(r'tf_F2.npy', np.array(tf_F2))
